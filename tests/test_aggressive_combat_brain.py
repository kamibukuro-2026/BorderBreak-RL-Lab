...
class TestAggressiveCombatBrainApproach:
    ...
    def test_approach_prioritized_over_hunt(self):
        ...
        visible = make_agent(agent_id=2, x=5, y=32, team=1)      # dist=7, 索敵内
        hunted  = make_agent(agent_id=3, x=5, y=10,  team=1,
                             detected=True)                        # dist=15, 範囲外 detected
...
class TestAggressiveCombatBrainHunt:
    ...
    def test_hunt_moves_toward_detected_enemy_below(self):
        """索敵外・detected 敵が下(dist=15) → HUNT → MOVE_DOWN"""
        enemy = make_agent(agent_id=2, x=5, y=40, team=1,
                           detected=True)   # dist=15 > 8
        action = self._decide([self.agent, enemy])
        assert action is Action.MOVE_DOWN

    def test_hunt_moves_toward_detected_enemy_above(self):
        """索敵外・detected 敵が上(dist=15) → HUNT → MOVE_UP"""
    ...
    def test_hunt_ignores_non_detected_enemy_outside_range(self):
        ...
        enemy = make_agent(agent_id=2, x=5, y=40, team=1,
                           detected=False)  # dist=15, 非detected
        action = self._decide([self.agent, enemy])
    ...
    def test_hunt_targets_nearest_when_multiple_detected(self):
        ...
        near = make_agent(agent_id=2, x=5, y=40, team=1,
                          detected=True)   # dist=15（下）
        far  = make_agent(agent_id=3, x=5, y=5,  team=1,
                          detected=True)   # dist=20（上）
    ...
    def test_hunt_detected_in_search_range_handled_as_approach(self):
        """detected=True でも索敵範囲内なら APPROACH として処理される"""
        # dist=7 は索敵内 → visible に入る → APPROACH（detected 参照前に分岐）
        enemy = make_agent(agent_id=2, x=5, y=32, team=1,
                           detected=True)  # dist=7, 索敵内
        action = self._decide([self.agent, enemy])
        # 索敵内なので APPROACH → MOVE_DOWN
        assert action is Action.MOVE_DOWN

    def test_hunt_not_triggered_when_visible_enemy_exists(self):
        """索敵内に敵がいれば、別の detected 敵は HUNT されない（APPROACH が優先）"""
        visible_enemy  = make_agent(agent_id=2, x=5, y=32, team=1)       # dist=7
        detected_enemy = make_agent(agent_id=3, x=5, y=10,  team=1,
                                    detected=True)                          # dist=15
