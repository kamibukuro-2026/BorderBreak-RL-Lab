...
├── renderer.py                # Simulation 描画ロジック（draw_simulation）
├── logger.py                  # CSV ログ保存ロジック（save_dev_logs）
├── main.py                    # 実行エントリポイント（10 vs 10 シミュレーション）
...
simulation.py       → constants, game_types, brain, agent, map_gen, renderer, logger
                      （テスト後方互換のため全シンボルを re-import）
renderer.py         → constants, game_types, agent（draw_simulation 関数）
logger.py           → （stdlib のみ。Simulation は TYPE_CHECKING のみ）
main.py             → constants, brain, agent, map_gen, simulation
