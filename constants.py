...
# マップ定数
# ─────────────────────────────────────────
CELL_SIZE_M    = 5     # 1マスのサイズ（メートル）
MAP_WIDTH_M    = 100   # マップ横幅（メートル）
MAP_HEIGHT_M   = 500   # マップ縦幅（メートル）
MAP_W          = MAP_WIDTH_M  // CELL_SIZE_M   # 20 セル
MAP_H          = MAP_HEIGHT_M // CELL_SIZE_M   # 100 セル
BASE_DEPTH     = 30 // CELL_SIZE_M  # ベースの奥行き（30m ÷ CELL_SIZE_M = 6 セル）
NUM_PLANTS     = 3     # プラント数
PLANT_RADIUS_M = 30    # プラント占拠範囲（メートル）
PLANT_RADIUS_C = PLANT_RADIUS_M / CELL_SIZE_M  # 6.0 セル
...
SEARCH_RANGE_C = SEARCH_RANGE_M / CELL_SIZE_M   # 16.0 セル
LOCKON_RANGE_M = 60      # ロックオン距離（メートル）
LOCKON_RANGE_C = LOCKON_RANGE_M / CELL_SIZE_M   # 12.0 セル
RESPAWN_STEPS  = 10      # 撃破からリスポーンまでのステップ数
MOVE_SPEED_MPS    = 21.9    # 標準BR移動速度（m/s、公式設定値）
CELLS_PER_STEP    = max(1, round(MOVE_SPEED_MPS / CELL_SIZE_M))  # 4 cells/step ≈ 20m/s
